...
class ColorIndents {
	...
	public init(): void {
		this.indentsClasses.forEach((indentClass) => {
			...
			elements.forEach((element) => {
				...
				indentElement.classList.add("indent-hidden");
	...
	/**
	 * Делает отступы видимыми
	 * @return { void }
	 */
	public indentsOn(): void {
		console.log('indentsOn');
		this.indentsElements.forEach((indents) => {
			indents.element.classList.remove("indent-hidden");
		});
	}

	/**
	 * Делает отступы невидимыми
	 * @return { void }
	 */
	public indentsOff(): void {
		console.log('indentsOff');
		this.indentsElements.forEach((indents) => {
			indents.element.classList.add("indent-hidden");
		});
	}
